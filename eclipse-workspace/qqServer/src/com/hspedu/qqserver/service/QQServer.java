...
public class QQServer {
	...
	//这里我们也可以使用ConcurrentHashMap代替HashMap,可以处理并发的集合，没有线程安全
	...
	private boolean checkUser(String userId,String passwd) {
		...
		//过关的验证方式
		if(user == null) {//说明userId没有存在validUsers的key
			return false;
		}
		if(!user.getPasswd().equals(passwd)) {//userId正确，但是密码错误
			return false;
		}
		return true;//密码正确，userId也正确
	}
		...
		try {
			...
			while(true) {//当和某个客户端连接后，会继续监听，因此while
				...
				if(checkUser(u.getUserId(),u.getPasswd())) {//登录通过
					...
					//创建一个线程，和客户端保持通信，该线程需要有socket对象

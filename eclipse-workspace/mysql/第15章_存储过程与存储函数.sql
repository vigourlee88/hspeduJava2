#第15章_存储过程与存储函数  stored procedure
#预先编译的SQL语句的封装
...
BEGIN
	SELECT * FROM employees;#存储过程
END $

DELIMITER ;


#2. 存储过程的调用使用 call,函数使用select 
...
BEGIN
	SELECT salary INTO empsalary  # salary 类型空间和empsalary要一样
	FROM employees
	WHERE last_name = empname;
END //
...
#4.存储过程、函数的修改,不影响存储过程或功能，只是修改相关特性characteristic

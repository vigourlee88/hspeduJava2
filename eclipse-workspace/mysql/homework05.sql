...
CREATE TABLE DEPARTMENT(
          departmentid VARCHAR(32) PRIMARY KEY, -- 001不是整数
          deptname VARCHAR(32) UNIQUE NOT NULL);


-- 班CLASS （班号classid，专业名subject，系名deptname，
-- 入学年份enrolltime，人数num）
CREATE TABLE `CLASS`(
          classid INT PRIMARY KEY,-- 是数字
          `subject` VARCHAR(32) NOT NULL DEFAULT '', 
          deptname VARCHAR(32) ,-- 外键字段，在表定义后指定
          enrolltime INT NOT NULL DEFAULT 2000,
          num INT NOT NULL DEFAULT 0,
	  FOREIGN KEY (deptname) REFERENCES  DEPARTMENT(deptname));-- 外键指定
...
CREATE TABLE hsp_student(
         ...
         classid INT , -- 外键，指向class 表的classid

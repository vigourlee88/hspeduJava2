...
SELECT * 
         FROM emp
         WHERE IFNULL(comm,0) > sal  -- comm为佣金，可能null
...
-- 还有既不是经理又不是办事员，但其薪金大于或等于2000的所有员工的详细资料
SELECT * 
        FROM emp
        WHERE (deptno = 10 AND job = 'MANAGER') 
        OR (deptno = 20 AND job = 'CLERK')
...
-- 9.找出各月倒数第3天受雇的所有员工
-- 提示方法  函数LAST_DAY(日期)，可以返回该日期所在月份的最后一天
...
SELECT REPLACE(ename,'A','a')
              FROM emp
-- 16.显示满10年服务年限的员工的姓名和受雇 日期  间隔INTERVAL
...
SELECT * 
      ...
      ORDER BY ename
-- 18.显示员工的姓名和受雇日期，
-- 根据其服务年限，将最老的员工排在最前面  ASC升序默认不写
SELECT ename,hiredate
           FROM emp
           ORDER BY hiredate 

-- 19.显示所有员工的姓名，工作和薪金，按工作降序排序，
-- 若工作相同则按薪金排序
SELECT ename,job,sal
             FROM emp
             ORDER BY job DESC,sal
-- 20.显示所有员工的姓名，加入公司的年份和月份，按受雇日期所在的月排序
-- 若月份相同则按最早年份的员工排在最前面 默认升序
SELECT ename,CONCAT(YEAR(hiredate),'-',MONTH(hiredate))
            FROM emp 
            ORDER BY MONTH(hiredate),YEAR(hiredate)
-- 21.显示在一个月为30天的情况所有员工的日薪金，
-- 忽略余数 则是向下取整 FLOOR ,保留小数则是 ROUND
...
SELECT *
         FROM emp
         WHERE ename LIKE '%A%'
-- 25.以年月日的方式显示所有员工的服务年限大概
-- 1.先求出工作了多少天
SELECT ename,FLOOR(DATEDIFF(NOW(),hiredate) / 365) AS '工作的年',
             FLOOR(DATEDIFF(NOW(),hiredate) % 365 / 31) AS '工作的月',
             DATEDIFF(NOW(),hiredate)% 31 AS '工作的天'
          FROM emp

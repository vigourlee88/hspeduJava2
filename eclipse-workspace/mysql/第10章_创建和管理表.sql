...
#如果创建表时没有指明使用的字符集，则默认使用表 所在的  数据库atguigudb,使用的字符集utf8mb3
...
#说明1：查询语句中字段的别名emp_id，lname，可以作为新创建的表的字段的名称。
...
#DOUBLE(10,2), 一共有10位，小数点后保留2位

ALTER TABLE myemp1
ADD phone_number VARCHAR(20) FIRST; #FIRST 添加到第一位

ALTER TABLE myemp1
ADD email VARCHAR(45) AFTER emp_name;#AFTER emp_name 指定添加到的位置


# 3.2 修改一个字段modify：数据类型、长度、默认值（略）一般不去改数据类型
...
# 3.4 删除一个字段drop column(列)
ALTER TABLE myemp1
DROP COLUMN my_email;

#4. 重命名表 rename table 表名 to
...
# 不同点：
#	TRUNCATE TABLE：清空，一旦执行此操作，表数据全部清除。同时，数据是不可以回滚的。

...
#建议  从sql优化的角度，建议多表查询时，每个字段前 都指明其所在的表

#5.可以给表起别名，在select 和where 中使用表的别名
#如果给表起了别名，一旦在select 或where 中使用了表名的话，则必须使用表的别名，而不能再使用表的原名
# 查询顺序是 from-> where ->select
...
WHERE e.department_id = d.department_id;#107个人，一个人没有部门,只有106条记录

#外连接 :合并具有同一列的两个以上的表的行，结果集中除了包含一个表与另一个表的匹配的行之外，
#        还查询到了左表 或 右表中不匹配的行
...
#SQL99语法中使用join...on 的方式实现多表的查询，这种方式也能解决外连接问题，MySQL支持此种方式
#SQL99语法如何实现多表的查询
...
#11.sql99语法的新特性2:using()括号中需要填入要指定的同名字段

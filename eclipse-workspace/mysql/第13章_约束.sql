...
ALTER TABLE 时  增加约束、删除约束


*/

#2. 如何查看表中的约束   
#系统自带的数据库information_schema   表名table_constraints 
...
VALUES(2,'abc@126.com');  #报错，last_name默认为null

UPDATE test1
SET last_name = NULL  #报错 not null
WHERE id = 1;
...
CREATE TABLE test2(
id INT UNIQUE,          #将此约束声明在字段后面称为 列级约束
last_name VARCHAR(15) , #not null无表约束，只有列约束
email VARCHAR(25),
salary DECIMAL(10,2),
...
#在创建唯一约束的时候，如果不给唯一约束命名，就默认和列名相同(id)。
...
-- 删除唯一约束只能通过 删除唯一索引的方式删除。
-- 删除时需要 指定唯一索引名，唯一索引名就和唯一约束名一样。
...
#5. primary key (主键约束)  用来唯一标识表中的一行记录
...
# 一个表最多只能有一个自增长列
# 当需要产生标识符或顺序值时，可设置自增长
# 自增长列约束的列必须是键列 (主键列，唯一键列)
# 自增约束的列的数据类型必须是整数类型
# 如果自增列指定了0和null，会在当前最大值的基础上自增
# 如果自增列手动指定了具体值，直接赋值为具体值

# 6.1 在CREATE TABLE时添加,AUTO_INCREMENT 必须绑定 PRIMARY KEY
CREATE TABLE test7(
id INT PRIMARY KEY AUTO_INCREMENT,
last_name VARCHAR(15) 
);
#开发中，一旦主键作用的字段上声明有AUTO_INCREMENT，
#则我们在添加数据时，就不要写主键，对应的字段去赋值了。
...
#重启服务器 cmd 管理员身份 net stop mysql80 net start mysql80
...
#在MySQL 8.0中新特性 - 自增变量的持久化
#将自增主键的计数器持久化到重做日志中

#MySQL 5.7 是放在  内存中维护的
CREATE TABLE test9(
id INT PRIMARY KEY AUTO_INCREMENT
);
...
#7.foreign key (外键约束) 限定某个表的某个字段的引用完整性，外键关联主键
...
ALTER TABLE emp1
DROP FOREIGN KEY fk_emp1_dept_id;#外键约束名去删除
...
#9.2 在ALTER TABLE添加约束default默认值
CREATE TABLE test12(
id INT,
last_name VARCHAR(15),
salary DECIMAL(10,2)

...
SHOW CREATE PROCEDURE add_double;#在命令行查看更好末尾加上\G
...
#调用  调用函数用户的是select ,调用存储过程用的是 call
SELECT get_count();
...
#3. 创建函数dept_sal() ,根据部门名，返回该部门的平均工资 需使用两张表
#如果系统重启后net start mysql80，需要重新设定
#创建函数前执行此语句，保证函数的创建会成功 加上函数特性
SET GLOBAL log_bin_trust_function_creators = 1;

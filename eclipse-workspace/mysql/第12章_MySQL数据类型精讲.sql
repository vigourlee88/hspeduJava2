...
SHOW VARIABLES LIKE 'character_%';#没有指明字符集，则系统默认
...
INSERT INTO test_int1(f1)#f1是字段
VALUES(12),(-12),(-128),(127);

SELECT * FROM test_int1;

#Out of range value for column 'f1' at row 1 超出数值范围
...
SELECT SUM(f1)
FROM test_double2;#SUM(f1)约等于1.1

SELECT SUM(f1) = 1.1, 1.1 = 1.1
FROM test_double2;#0 ,1
...
FROM test_bit1;#BIN 2进制 HEX 16进制
...
#7.2 VARCHAR类型可变长度  必须指定长度varchar()
...
INSERT INTO test_set (s) VALUES ('A'), ('A,B');#可以任意选一个或多个
...
FROM test_binary1;#f2 固定长度2，f4可变的

#10.2 Blob类型 可容纳可变数量的数据 比如图片，音频，视频
...
#11. JSON类型 数据交换格式
#可以将javascript对象中表示的一组数据转换成字符串，然后可以在网络或程序之间传递
...
################################
#小结
#定义数据类型时，整数用int,小数一定用定点类型decimal(M,D),日期与时间就用datetime

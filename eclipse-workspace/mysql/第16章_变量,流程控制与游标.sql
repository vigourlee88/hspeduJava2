...
SELECT @@character_set_client; #没指定全局或会话变量，先查询会话系统变量，再查询全局系统变量

#1.4 修改系统变量的值 
#修改mysql配置文件，继而修改mysql系统变量的值(该方法需要重启mysql服务)
#在服务器运行期间使用set命令重新设置系统变量的值
...
#针对于当前会话是有效的，一旦结束会话(即创建新连接)，重新建立起新的会话，就失效了。
...
③ 局部变量：只能使用在begin和end语块，存储过程和存储函数中的。
...
③ DECLARE的方式声明的局部变量必须声明在BEGIN中的 首行 的位置。
...
#错误代码： 1364
#Field 'email' doesn't have a default value not null
...
BEGIN	
	#情况1：
	#声明局部变量
	#declare stu_name varchar(15); #默认值是''
	...
	IF age > 40
		THEN SELECT 'laoren' ;
	ELSEIF age > 18
		THEN SELECT 'qingnian';
	ELSEIF age > 8
		THEN SELECT 'shaonian';
	ELSE
		SELECT 'xinshenger';
	END IF;

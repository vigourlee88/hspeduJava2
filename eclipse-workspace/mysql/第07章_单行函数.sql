...
#2. 字符串函数
#返回第一个字符，返回字符的长度，字符集字节的长度，hello1个字节是一个字符即5，我们一个汉字是3个字节即6
SELECT ASCII('Abcdfsf'),CHAR_LENGTH('hello'),CHAR_LENGTH('我们'),
LENGTH('hello'),LENGTH('我们')
FROM DUAL;

# xxx worked for yyy  声明多个字符串 拼接
...
#字符串的索引是从1开始的！2指的是e,2开始3个替换成，hello中ll替换成 mm,没有就替换失败，返回原来的值
SELECT INSERT('helloworld',2,3,'aaaaa'),REPLACE('hello','ll','mm'),REPLACE('hello','lol','mmm')
FROM DUAL;
...
# LPAD:实现右对齐效果
# RPAD:实现左对齐效果
#向左 24000.00 8位补2个*，变成10位
SELECT employee_id,last_name,LPAD(salary,10,'*')
FROM employees;

#trim 去除首尾空格, LTRIM去除头部的空格
SELECT CONCAT('---',LTRIM('    h  el  lo   '),'***'),
TRIM('oo' FROM 'ooheollo')#去除首尾的oo
FROM DUAL;

SELECT REPEAT('hello',4),LENGTH(SPACE(5)),STRCMP('abc','abe')#strcmp比较字符串大小
FROM DUAL;

#找到子字符串从2开始取2个，locate 定位'lll'首次出现在'hello'的位置
SELECT SUBSTR('hello',2,2),LOCATE('ll','hello')
FROM DUAL;

#elt返回指定位置2的字符串,field 找到'mm'字符串在 后面字符串列表中第一次出现的位置3
SELECT ELT(2,'a','b','c','d'),FIELD('mm','gg','jj','mm','dd','mm'),
FIND_IN_SET('mm','gg,mm,jj,dd,mm,gg')
FROM DUAL;

#nullif 相等返回null,不相等则返回LENGTH(first_name)
...
FROM_UNIXTIME(1635173853),FROM_UNIXTIME(1633061552)#返回日期，当前时间
FROM DUAL;
...
SELECT *
FROM employees
WHERE hire_date = '1993-01-13';#必须和date写法一样
...
SELECT GET_FORMAT(DATE,'USA')#格式化类型'USA'
FROM DUAL;
...
#IFNULL(commission_pct,0) 假如commission_pct不是null,就输出它本身，
...
# PASSWORD()在mysql8.0中弃用。MD5,SHA不可逆的加密
SELECT MD5('mysql'),SHA('mysql'),MD5(MD5('mysql'))
FROM DUAL;

#ENCODE()\DECODE() 加密，解密在mysql8.0中弃用。
...
#四舍五入，保留2位，即123.13，即123，-2就往前考虑，123
...
#BENCHMARK()用于测试表达式的执行效率/时间执行0.026sec
SELECT BENCHMARK(100000,MD5('mysql'))
FROM DUAL;
# CONVERT():可以实现字符集的转换  utf8mb4,拿4个字节表示一个字符

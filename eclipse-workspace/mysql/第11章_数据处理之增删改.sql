...
#注意说明：emp1表中要添加数据的字段的长度不能低于employees表中查询的字段的长度。
...
#修改数据时，是可能存在不成功的情况的。（可能是由于外键约束的影响造成的）
UPDATE employees
...
#4. MySQL8的新特性：计算列:某一列的值是通过别的列计算得来的
USE atguigudb;

CREATE TABLE test1(
a INT,
b INT,
c INT GENERATED ALWAYS AS (a + b) VIRTUAL  #VIRTUAL虚拟的，字段c即为计算列
);

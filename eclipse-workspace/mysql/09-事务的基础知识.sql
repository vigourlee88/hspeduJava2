...
#3.2 如果关闭自动提交？
#方式1：
SET autocommit = FALSE; #针对于 DML操作 是有效的，对 DDL操作 是无效的。
...
BEGIN;#开启事务
INSERT INTO user3 VALUES('张三'); #加begin后，此时不会自动提交数据
COMMIT;

BEGIN; #开启一个新的事务
INSERT INTO user3 VALUES('李四'); #此时会添加数据成功，但是不会自动提交数据
INSERT INTO user3 VALUES('李四'); #受主键的影响，不能添加成功
ROLLBACK;#未提交，回滚后返回最后一次提交处  张三

SELECT * FROM user3;

#情况2：
SET autocommit = TRUE;
TRUNCATE TABLE user3;  #DDL操作会自动提交数据，不受autocommit变量的影响。会提交数据
...
ROLLBACK;#返回最近一次commit

SELECT * FROM user3;


#情况3：
TRUNCATE TABLE user3;#清空表中所有数据

SELECT * FROM user3;

SELECT @@completion_type;#设置系统变量
